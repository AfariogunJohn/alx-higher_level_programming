...
an N×N chessboard. Write a program that solves the N queens problem.
    ...
    Usage: nqueens N
        ...
        status 1 where N must be an integer greater or equal to 4
    ...
    The program should print every possible solution to the problem
        ...
        Format: see example
        You don’t have to print the answer in a specific order
...
def recursive_solve(board, row, queens, answer):
    ...
    Args:
        ...
        answer (list): A list of lists of answer.
    Returns:
        answer
    """
    if queens == len(board):
    ...
    for c in range(len(board)):
        if board[row][c] == " ":
            ...
            temp[row][c] = "Q"
            out(temp, row, c)
            answer = recursive_solve(temp, row + 1,
                                     queens + 1, answer)
